...
comes up. A card is *mature* once its gap exceeds the deck size. Number cards can also quiz **Position**, **Shoots** and **Country** (toggles, all on by default, saved under `chdle:anki:fields`); a quizzed field is hidden from the card and the card only counts as right when every answer is. Progress lives in `localStorage`
