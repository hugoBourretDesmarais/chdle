import { NAMES, NHL_GAMES, SCOPES } from './data.js'
...
// "arc" throughout is the roster scope: '' for the full camp roster, 'nhl'
// for players with at least one NHL game.
function poolFor(arcLimit) {
  if (arcLimit === 'nhl') return NAMES.filter((_, i) => NHL_GAMES[i] > 0)
  return NAMES
}

function answerFor(day, arcLimit) {
  const pool = poolFor(arcLimit)
  return pool[hashString('chdle:' + day) % pool.length]
}
...
const isKnownArc = a => !a || SCOPES.includes(a)
...
export default {
  async fetch(request, env) {
    ...
    if (request.method === 'GET' && url.pathname === '/count') {
      ...
      const row = await env.DB.prepare(
        'SELECT count FROM ch_solves WHERE day = ? AND arc_limit = ?'
    ...
    if (request.method === 'POST' && url.pathname === '/solve') {
      ...
      const first = await env.DB.prepare(
        'INSERT OR IGNORE INTO ch_counted (day, arc_limit, ip_hash) VALUES (?, ?, ?)'
      ).bind(day, arcLimit, hash).run()
      const isNew = (first.meta?.changes ?? 0) > 0
      if (isNew) {
        await env.DB.prepare(
          `INSERT INTO ch_solves (day, arc_limit, count) VALUES (?, ?, 1)
    ...
    if (request.method === 'POST' && url.pathname === '/register') {
      ...
      try {
        await env.DB.batch([
          ...
          env.DB.prepare('INSERT OR IGNORE INTO ch_standings (player_id) VALUES (?)').bind(id),
        ])
      } catch (err) {
    ...
    if (request.method === 'POST' && url.pathname === '/result') {
      ...
      const existing = await env.DB.prepare(
        'SELECT day FROM ch_results WHERE player_id = ? AND day = ?'
      ...
      await env.DB.prepare('INSERT OR IGNORE INTO ch_standings (player_id) VALUES (?)').bind(player.id).run()
      const st = await env.DB.prepare(
        'SELECT wins, total_guesses, max_streak FROM ch_standings WHERE player_id = ?'
      ).bind(player.id).first() || { wins: 0, total_guesses: 0, max_streak: 0 }

      await env.DB.prepare(
        `INSERT INTO ch_results (player_id, day, arc_limit, guesses, solved_at)
         VALUES (?, ?, ?, ?, ?)`
      ...
      const recent = await env.DB.prepare(
        'SELECT day, arc_limit FROM ch_results WHERE player_id = ? ORDER BY day DESC LIMIT 90'
      ).bind(player.id).all()
      ...
      await env.DB.prepare(
        `UPDATE ch_standings SET wins = wins + 1, total_guesses = total_guesses + ?,
    ...
    if (request.method === 'GET' && url.pathname === '/leaderboard') {
      ...
      const rows = await env.DB.prepare(
        `SELECT p.name, st.wins, st.total_guesses, st.streak, st.max_streak, st.last_win_day,
                st.last_arc_limit
         FROM ch_standings st JOIN players p ON p.id = st.player_id
    ...
    if (request.method === 'GET' && url.pathname === '/me') {
      ...
      if (!player) return json({ error: 'not signed in' }, 401, headers)
      const st = await env.DB.prepare(
        'SELECT wins, total_guesses, streak, max_streak, last_win_day FROM ch_standings WHERE player_id = ?'
