...
-- the common account tables, everything game-specific is prefixed ch_.
-- Solve counter: one row per (game day, roster scope = nhl, '' for none), incremented on a
-- verified solve. Kept separate from players so the counter works for people
-- who never sign up.
CREATE TABLE IF NOT EXISTS ch_solves (
  day         TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS ch_counted (
  day       TEXT NOT NULL,
  arc_limit TEXT NOT NULL DEFAULT '',
...
CREATE TABLE IF NOT EXISTS ch_results (
  player_id   TEXT NOT NULL,
  day         TEXT NOT NULL,
  arc_limit   TEXT NOT NULL DEFAULT '',
...
CREATE TABLE IF NOT EXISTS ch_standings (
...
CREATE INDEX IF NOT EXISTS idx_ch_standings_wins ON ch_standings(wins DESC);
CREATE INDEX IF NOT EXISTS idx_ch_standings_streak ON ch_standings(max_streak DESC);
CREATE INDEX IF NOT EXISTS idx_ch_results_day ON ch_results(day, guesses);
